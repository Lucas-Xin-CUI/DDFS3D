...
subroutine dis_regular_dd3d_qua_num (xx1,yy1,zz1,xx2,yy2,zz2,xx3,yy3,zz3,xx4,yy4,zz4,gpx,gpy,gpz,E,v,M,IEvect,JEvect,COEFF)
implicit none 

real*8::E,v,cond,l1,l2,l3,p,area,x1,y1,z1,x2,y2,z2,x3,y3,z3,x4,y4,z4,xx1,yy1,zz1,xx2,yy2,zz2,xx3,yy3,zz3,xx4,yy4,zz4
real*8::gpx,gpy,gpz,gpxx,gpyy,gpzz,vax,vay,vaz,vbx,vby,vbz,vcx,vcy,vcz,vdx,vdy,vdz
real*8::VecIJ_x,VecIJ_y,VecIJ_z,localx,localy,localz,IEvect(3,3),JEvect(3,3),COEFF(3,3)
real*8::Ix,Iy,Iz,Ixx,Ixy,Ixz,Iyz,Iyy,Izz,kesi,yita,A,B,U(3,3),JI(3,3)
real*8::n1,n2,n3,n4,jaco_11,jaco_12,jaco_21,jaco_22,jaco
integer::i,j,k,M
...
!Calculate Ixx  
    ...
    do i=1,M
        do j=1,M 
             ...
             Ixx=Ixx+B*(3*A**(-2.5)*(localx-kesi)**2-A**(-1.5))
        end do 
    end do      
...
!Calculate Ixy  
    ...
    do i=1,M
        do j=1,M
             ...
             Ixy=Ixy+B*3*A**(-2.5)*(localx-kesi)*(localy-yita)
        end do 
    end do
...
!Calculate Ixz  
    ...
    do i=1,M
        do j=1,M 
             ...
             Ixz=Ixz+B*3*A**(-2.5)*(localx-kesi)*localz
        end do 
    end do  
...
!Calculate Iyy  
    ...
    do i=1,M
        do j=1,M
             ...
             Iyy=Iyy+B*(3*A**(-2.5)*(localy-yita)**2-A**(-1.5))
        end do 
    end do    
...
!Calculate Iyz  
    ...
    do i=1,M
        do j=1,M
             ...
             Iyz=Iyz+B*3*A**(-2.5)*(localy-yita)*localz
        end do 
    end do   
...
!Calculate Izz    
    ...
    do i=1,M
        do j=1,M
             ...
             Izz=Izz+B*(3*A**(-2.5)*localz**2-A**(-1.5))
        end do    
    end do 
...
!*****************************************************************   
      U(1,1)=  2.0 * ( 1.0 - v ) * Iz - localz*Ixx
      U(1,2)=                         - localz*Ixy
      U(1,3)= -( 1.0 - 2.0 * v ) * Ix - localz*Ixz
      U(2,1)=                         - localz*Ixy
      U(2,2)=  2.0 * ( 1.0 - v ) * Iz - localz*Iyy
      U(2,3)= -( 1.0 - 2.0 * v ) * Iy - localz*Iyz
      U(3,1)=  ( 1.0 - 2.0 * v ) * Ix - localz*Ixz
      U(3,2)=  ( 1.0 - 2.0 * v ) * Iy - localz*Iyz
      U(3,3)=  2.0 * ( 1.0 - v ) * Iz - localz*Izz
...
end subroutine dis_regular_dd3d_qua_num 
     

subroutine dis_singular_dd3d_qua_num (xx1,yy1,zz1,xx2,yy2,zz2,xx3,yy3,zz3,xx4,yy4,zz4,E,v,M,IEvect,COEFF)

real*8::E,v,G,cons,l1,l2,l3,p,area,x(5),y(5),z(5),xx1,yy1,zz1,xx2,yy2,zz2,xx3,yy3,zz3,xx4,yy4,zz4,xx(4),yy(4),zz(4)
real*8::IEvect(3,3),JEvect(3,3),COEFF(3,3),A,B,yip,xm,ym,r,localz,gpx,gpy,gpz
real*8::Ix,Iy,Iz,Ixx,Ixy,Ixz,Iyz,Iyy,Izz,U(3,3),JI(3,3)
integer::M,I,J
real*8,allocatable::pw(:,:)
...
Iyy=0.
!**************************************************************************************        
! For self-influence coefficients, only first-order partial direvatives are needed.  
!**************************************************************************************
do 100 i=1,3 
    l1=sqrt(x(i)**2+y(i)**2+z(i)**2)
    l2=sqrt((x(i+1)-x(i))**2+(y(i+1)-y(i))**2+(z(i+1)-z(i))**2)
    l3=sqrt(x(i+1)**2+y(i+1)**2+z(i+1)**2)
    p=(l1+l2+l3)/2.
    ...
    do j=1,m 
        yip=0.5*(pw(j,1)+1) 
        xm=(1-yip)*x(i)+yip*x(i+1)
        ym=(1-yip)*y(i)+yip*y(i+1)
        r=sqrt(xm**2+ym**2) 
        A=A+pw(j,2)*area*xm*log(r)/r**3
        B=B+pw(j,2)*area*ym*log(r)/r**3  
    end do
        Ix=Ix+A
        Iy=Iy+B
100 continue  
...
end subroutine dis_singular_dd3d_qua_num


subroutine stress_regular_dd3d_qua_num_part1 (xx1,yy1,zz1,xx2,yy2,zz2,xx3,yy3,zz3,xx4,yy4,zz4,gpx,gpy,gpz,E,v,M,IEvect,JEvect,COEFF)
implicit none 

real*8::E,v,G,cons,l1,l2,l3,p,area,x1,y1,z1,x2,y2,z2,x3,y3,z3,x4,y4,z4,xx1,yy1,zz1,xx2,yy2,zz2,xx3,yy3,zz3,xx4,yy4,zz4
real*8::gpx,gpy,gpz,gpxx,gpyy,gpzz,vax,vay,vaz,vbx,vby,vbz,vcx,vcy,vcz,vdx,vdy,vdz,n1,n2,n3,n4
real*8::VecIJ_x,VecIJ_y,VecIJ_z,localx,localy,localz,IEvect(3,3),JEvect(3,3),COEFF(3,3),jaco_11,jaco_12,jaco_21,jaco_22,jaco
real*8::Ix,Iy,Iz,Ixx,Ixy,Ixz,Iyz,Iyy,Izz,Ixxx,Ixxy,Ixxz,Iyyy,Iyyx,Iyyz,Izzx,Izzy,Izzz,Ixyz,kesi,yita,A,B,S(6,3),JI(3,3),Trac(3)
integer::i,j,k,M
real*8,allocatable::pw(:,:)

!********************************************************************************************************************************
G=E/2./(1+v)
cons=1./4./3.141592653/(1-v)*G
...
call Gauss_ponits_weights(M,pw)
!*****************************************************************************
! First-order partial derivatives are unnecessary to evaluate stresses
!******************************************************************************
!!Calculate Ix   
!   Ix=0.
!    do i=1,M
!        JJ=M+2-i
!        allocate (pw(JJ,2))
!        call Gauss_ponits_weights(JJ,pw)
!        do j=1,JJ
!             kesi=x1+0.5*(x2-x1)*(1+pw(i,1))+0.25*(x3-x1)*(1-pw(i,1))*(1+pw(j,1))
!             yita=y1+0.5*(y2-y1)*(1+pw(i,1))+0.25*(y3-y1)*(1-pw(i,1))*(1+pw(j,1))
!             A=(localx-kesi)**2+(localy-yita)**2+localz**2
!             B=2*Area*pw(i,2)*pw(j,2)*(1-pw(i,1))/8.
!             
!             Ix=Ix-B*A**(-1.5)*(localx-kesi)
!                       
!        end do 
...
!Calculate Ixxx 
    ...
    do i=1,M
        do j=1,M
             ...
             Ixxx=Ixxx+B*(-15*A**(-3.5)*(localx-kesi)**3+9*A**(-2.5)*(localx-kesi))
        end do
    end do 
...
!Calculate Ixxy 
    ...
    do i=1,M
        do j=1,M
             ...
             Ixxy=Ixxy+B*(-15*A**(-3.5)*(localx-kesi)**2*(localy-yita)+3*A**(-2.5)*(localy-yita))
        end do 
    end do     
 ...
 !Calculate Ixxz 
    ...
    do i=1,M
        do j=1,M
             ...
             Ixxz=Ixxz+B*(-15*A**(-3.5)*(localx-kesi)**2*localz+3*A**(-2.5)*localz)
        end do 
    end do     
 ...
 !Calculate Iyyx 
    ...
    do i=1,M
        do j=1,M
             ...
             Iyyx=Iyyx+B*(-15*A**(-3.5)*(localy-yita)**2*(localx-kesi)+3*A**(-2.5)*(localx-kesi))
        end do 
    end do     
 ...
 !Calculate Iyyy 
    ...
    do i=1,M
        do j=1,M
             ...
             Iyyy=Iyyy+B*(-15*A**(-3.5)*(localy-yita)**3+9*A**(-2.5)*(localy-yita))
        end do 
    end do  
...
!Calculate Iyyz 
    ...
    do i=1,M
        do j=1,M
             ...
             Iyyz=Iyyz+B*(-15*A**(-3.5)*(localy-yita)**2*localz+3*A**(-2.5)*localz)
        end do 
    end do      
...
!Calculate Izzx 
    ...
    do i=1,M
        do j=1,M
             ...
             Izzx=Izzx+B*(-15*A**(-3.5)*localz**2*(localx-kesi)+3*A**(-2.5)*(localx-kesi))
        end do 
    end do   
...
!Calculate Izzy 
    ...
    do i=1,M
        do j=1,M
             ...
             Izzy=Izzy+B*(-15*A**(-3.5)*localz**2*(localy-yita)+3*A**(-2.5)*(localy-yita))
        end do
    end do     
...
!Calculate Izzz 
    ...
    do i=1,M
        do j=1,M
             ...
             Izzz=Izzz+B*(-15*A**(-3.5)*localz**3+9*A**(-2.5)*localz)
        end do
    end do     
...
!Calculate Ixyz 
    ...
    do i=1,M
        do j=1,M
             ...
             Ixyz=Ixyz+B*(-15*A**(-3.5)*(localx-kesi)*(localy-yita)*localz)
        end do 
    end do  
...
! Calculate influence coefficients 
!************************************************************
      S(1,1) =     + ( 2.0D0           ) * IXZ - LOCALZ * IXXX
      S(1,2) =     + ( 2.0D0 * V       ) * IYZ - LOCALZ * IXXY
      S(1,3) = IZZ + (1.0D0 - 2.0D0 * V) * IYY - LOCALZ * IXXZ
      S(2,1) =     + ( 2.0D0 * V       ) * IXZ - LOCALZ * Iyyx
      S(2,2) =     + ( 2.0D0           ) * IYZ - LOCALZ * IYYY
      S(2,3) = IZZ + (1.0D0 - 2.0D0 * V) * IXX - LOCALZ * IYYZ
      S(3,1) =     + (0.0D0            )       - LOCALZ * Izzx
      S(3,2) =     + (0.0D0            )       - LOCALZ * Izzy
      S(3,3) = IZZ + (0.0D0            )       - LOCALZ * IZZZ
      S(4,1) =     + (1.0D0 - V        ) * IYZ - LOCALZ * IXXY
      S(4,2) =     + (1.0D0 - V        ) * IXZ - LOCALZ * Iyyx
      S(4,3) =     - (1.0D0 - 2.0D0 * V) * IXY - LOCALZ * IXYZ
      S(5,1) =     + ( - V             ) * IXY - LOCALZ * IXYZ
      S(5,2) = IZZ + ( + V             ) * IXX - LOCALZ * IYYZ
      S(5,3) =     + (0.0D0            )       - LOCALZ * Izzy
      S(6,1) = IZZ + ( + V             ) * IYY - LOCALZ * IXXZ
      S(6,2) =     + ( - V             ) * IXY - LOCALZ * IXYZ
      S(6,3) =     + (0.0D0            )       - LOCALZ * Izzx
...
end subroutine stress_regular_dd3d_qua_num_part1


subroutine stress_regular_dd3d_qua_num_part3 (xx1,yy1,zz1,xx2,yy2,zz2,xx3,yy3,zz3,xx4,yy4,zz4,gpx,gpy,gpz,E,v,M,IEvect,JEvect,COEFF)
implicit none 

real*8::E,v,G,cons,l1,l2,l3,p,area,x1,y1,z1,x2,y2,z2,x3,y3,z3,x4,y4,z4,xx1,yy1,zz1,xx2,yy2,zz2,xx3,yy3,zz3,xx4,yy4,zz4
real*8::gpx,gpy,gpz,gpxx,gpyy,gpzz,vax,vay,vaz,vbx,vby,vbz,vcx,vcy,vcz,vdx,vdy,vdz,n1,n2,n3,n4
real*8::VecIJ_x,VecIJ_y,VecIJ_z,localx,localy,localz,IEvect(3,3),JEvect(3,3),COEFF(6,3),jaco_11,jaco_12,jaco_21,jaco_22,jaco
real*8::Ix,Iy,Iz,Ixx,Ixy,Ixz,Iyz,Iyy,Izz,Ixxx,Ixxy,Ixxz,Iyyy,Iyyx,Iyyz,Izzx,Izzy,Izzz,Ixyz,kesi,yita,A,B,S(6,3),JI(3,3),TRAC(3)
integer::i,j,k,M
real*8,allocatable::pw(:,:)
...
end subroutine stress_regular_dd3d_qua_num_part3


subroutine stress_singular_dd3d_qua_num (xx1,yy1,zz1,xx2,yy2,zz2,xx3,yy3,zz3,xx4,yy4,zz4,EE,v,M,IEvect,COEFF)
implicit none 

real*8::EE,v,GG,cons,l1,l2,l3,p,area,x(5),y(5),z(5),xx1,yy1,zz1,xx2,yy2,zz2,xx3,yy3,zz3,xx4,yy4,zz4,xx(4),yy(4),zz(4)
real*8::JEvect(3,3),IEvect(3,3),COEFF(3,3),A,B,C,D,E,F,G,H,I,J,yip,xm,ym,r,localz,gpx,gpy,gpz
real*8::Ix,Iy,Iz,Ixx,Ixy,Ixz,Iyz,Iyy,Izz,Ixxx,Ixxy,Ixxz,Iyyy,Iyyx,Iyyz,Izzx,Izzy,Izzz,Ixyz,S(6,3),JI(3,3),TRAC(3)
integer::ii,jj,M
real*8,allocatable::pw(:,:)
...
Ixyz=0.
!**************************************************************************************        
! For self-influence coefficients, only sencond-order partial direvatives are needed.  
!**************************************************************************************
do 100 ii=1,4
    ...
    do jj=1,m
        ...
        r=sqrt(xm**2+ym**2) 
        A=A-pw(jj,2)*area/r**3
        B=B-pw(jj,2)*area*xm**2/r**5
        C=C-pw(jj,2)*area*xm*ym/r**5
        !D=D-pw(jj,2)*0.5*area*xm**3/r**6
        !E=E-pw(jj,2)*0.5*area*xm/r**4
        !F=F-pw(jj,2)*0.5*area*xm**2*ym/r**6
        G=G-pw(jj,2)*area*ym**2/r**5
        !H=H-pw(jj,2)*0.5*area*ym**3/r**6
        !I=I-pw(jj,2)*0.5*area*ym/r**4
        !J=J-pw(jj,2)*0.5*area*ym**2*xm/r**6
    end do
...
end subroutine stress_singular_dd3d_qua_num
