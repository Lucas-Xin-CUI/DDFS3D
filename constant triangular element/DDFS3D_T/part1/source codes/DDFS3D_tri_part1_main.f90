!*********************************************************************************************
!Part 1: Solve for the displacement discontinuities or fictitious stresses at the centriod of each element.
!*********************************************************************************************

program DDFS3D_tri_part1
 
use prog_bar
use subs_dd3d_tri_num
use subs_dd3d_tri_ana
...
real*8 ::E,v,KN,KS,x1,y1,z1,x2,y2,z2,x3,y3,z3,xx1,yy1,zz1,xx2,yy2,zz2,xx3,yy3,zz3,gpx,gpy,gpz,COEFF1(3,3),COEFF2(3,3),IEvect(3,3),JEvect(3,3)
...
write (*,'(A87)')'**************************************************************************************'
write (*,'(A64)')'************		        DDFS3D_tri_part1		             **********'
write (*,'(A85)')'************	             Algorithm developed by                          **********'
write (*,'(A85)')'************	   Kuriyama et al.(1993 & 1995), Tang et al. (2019)          **********'
write (*,'(A64)')'************			and Wong and Cui (2021)		             **********'
write (*,'(A82)')'************	      Codes developed by Wong and Cui (2021)	             **********'
write (*,'(A87)')'************ incorporating the contribution of Kuriyama et al.(1993 & 1995) **********'
write (*,'(A72)')'************	          DDFS3D (version 1.0), 31-5-2021		     **********'
write (*,'(A87)')'**************************************************************************************'
...
allocate(NOD(NNOD,3),VOB(NELE,3),COE(3*NELE,3*NELE),X(3*NELE),B(3*NELE),ELE(NELE,3),E_type(NELE),SOB(NELE,3),nor(NELE,3),gx(NELE),gy(NELE),gz(NELE))
...
!$omp parallel do default(none) shared(COE,nor,AD1,progress,gx,gy,gz) shared(NELE,NOD,ELE,SOB,E_type,Self_T,E,v,Pxx,Pyy,Pzz,Pxy,Pxz,Pyz) &
        ...
        if (SOB(i,1)==0.and.SOB(i,2)==0.and.SOB(i,3)==0) then 
			if (i/=j)then
                if (E_type(j)==0)then
					...
					call stress_regular_dd3d_tri_num_part1 (xx1,yy1,zz1,xx2,yy2,zz2,xx3,yy3,zz3,gpx,gpy,gpz,E,v,M,IEvect,JEvect,COEFF1)
                end if    
				...
				if (E_type(j)==0)then
					...
					if (Self_T==0)then
						call diag_stress_dd3d_tri_ana (xx1,yy1,zz1,xx2,yy2,zz2,xx3,yy3,zz3,E,v,IEvect,COEFF1)
					else 
						call stress_singular_dd3d_tri_num (xx1,yy1,zz1,xx2,yy2,zz2,xx3,yy3,zz3,E,v,M,IEvect,COEFF1)
					end if 
				end if 
				COE(3*(i-1)+1,3*(j-1)+1)=COEFF1(1,1)
				COE(3*(i-1)+1,3*(j-1)+2)=COEFF1(1,2)
            ...
            if (i/=j)then  
			    if (E_type(j)==0)then
                    ...
                    call dis_regular_dd3d_tri_num (xx1,yy1,zz1,xx2,yy2,zz2,xx3,yy3,zz3,gpx,gpy,gpz,E,v,M,IEvect,JEvect,COEFF1)
				end if 
                COE(3*(i-1)+1,3*(j-1)+1)=COEFF1(1,1)
                COE(3*(i-1)+1,3*(j-1)+2)=COEFF1(1,2)
                COE(3*(i-1)+1,3*(j-1)+3)=COEFF1(1,3)           
			    ...
			    if (E_type(j)==0)then
				    ...
				    if (Self_T==0)then
					    call diag_dis_dd3d_tri_ana (xx1,yy1,zz1,xx2,yy2,zz2,xx3,yy3,zz3,E,v,IEvect,COEFF1)
				    else 
					    call dis_singular_dd3d_tri_num (xx1,yy1,zz1,xx2,yy2,zz2,xx3,yy3,zz3,E,v,M,IEvect,COEFF1)
				    end if 
			    end if 
                COE(3*(i-1)+1,3*(j-1)+1)=COEFF1(1,1)
                COE(3*(i-1)+1,3*(j-1)+2)=COEFF1(1,2)
            ...
            if (i/=j)then
				if (E_type(j)==0)then
					...
					call stress_regular_dd3d_tri_num_part1 (xx1,yy1,zz1,xx2,yy2,zz2,xx3,yy3,zz3,gpx,gpy,gpz,E,v,M,IEvect,JEvect,COEFF1)  
					call           dis_regular_dd3d_tri_num (xx1,yy1,zz1,xx2,yy2,zz2,xx3,yy3,zz3,gpx,gpy,gpz,E,v,M,IEvect,JEvect,COEFF2) 
				end if
			    if (SOB(i,1)==0)then 
					COE(3*(i-1)+1,3*(j-1)+1)=COEFF1(1,1)
					COE(3*(i-1)+1,3*(j-1)+2)=COEFF1(1,2)
				...
				if (E_type(j)==0)then
					...
					if (Self_T==0)then
						call diag_stress_dd3d_tri_ana (xx1,yy1,zz1,xx2,yy2,zz2,xx3,yy3,zz3,E,v,IEvect,COEFF1)
						call    diag_dis_dd3d_tri_ana (xx1,yy1,zz1,xx2,yy2,zz2,xx3,yy3,zz3,E,v,IEvect,COEFF2)
					else 
					    call stress_singular_dd3d_tri_num (xx1,yy1,zz1,xx2,yy2,zz2,xx3,yy3,zz3,E,v,M,IEvect,COEFF1)
						call    dis_singular_dd3d_tri_num (xx1,yy1,zz1,xx2,yy2,zz2,xx3,yy3,zz3,E,v,M,IEvect,COEFF2)
					end if 
				end if 
...
!$omp end parallel do 
!*****************************************************************************    
! Superimpose the normal and tangential stiffness coefficients
!*****************************************************************************
...
!$omp end parallel do
!**************************************************************
! Evaluate the external loading vector on righ-hand side 
!**************************************************************
...
write (41,'(A87)')'**************************************************************************************'
write (41,'(A64)')'************		        DDFS3D_tri_part1		             **********'
write (41,'(A85)')'************	             Algorithm developed by                          **********'
write (41,'(A85)')'************	   Kuriyama et al.(1993 & 1995), Tang et al. (2019)          **********'
write (41,'(A64)')'************			and Wong and Cui (2021)		             **********'
write (41,'(A82)')'************	      Codes developed by Wong and Cui (2021)	             **********'
write (41,'(A87)')'************ incorporating the contribution of Kuriyama et al.(1993 & 1995) **********'
write (41,'(A72)')'************	          DDFS3D (version 1.0), 31-5-2021		     **********'
write (41,'(A87)')'**************************************************************************************'
write (41,'(2A)')' Title: ',Title
...
write (41,'(A60)')'Coordinates of center of gravity and DD/FS of each element:'
write (41,'(A84)')' No.	     Gx            Gy            Gz           Dx/Px         Dy/Py         Dz/Pz '
do i=1,NELE
